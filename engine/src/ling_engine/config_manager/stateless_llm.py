...
class OpenAIConfig(OpenAICompatibleConfig):
    ...
    tier: int | None = Field(None, alias="tier")
    interrupt_method: Literal["system", "user"] = Field(
        "system", alias="interrupt_method"
    )

    # OpenAI-specific descriptions
    _OPENAI_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "tier": Description(
            en="Priority tier for OpenAI API (1-5, higher number = higher priority). Only works with official OpenAI API.",
            zh="OpenAI API 的优先级层级 (1-5，数字越高优先级越高)。仅适用于官方 OpenAI API。",
        ),
    }

    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        **OpenAICompatibleConfig.DESCRIPTIONS,
        **_OPENAI_DESCRIPTIONS,
    }

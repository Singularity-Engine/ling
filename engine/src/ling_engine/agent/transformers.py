            ...
            async for sentence in sentence_stream:
                ...
                if not any(
                    ...
                    if expressions:
                        ...
                        logger.debug(f"❌ 动作提取器：未在句子中找到表情标记")
                else:
                    logger.debug(f"⏭️ 句子包含特殊标记，跳过表情提取")
    ...
    def decorator(
        ...
        async def wrapper(*args, **kwargs) -> AsyncIterator[SentenceOutput]:
            ...
            if tts_preprocessor_config is None:
                # 直接使用默认的过滤行为，不创建完整的配置对象
                use_ignore_brackets = True
                use_ignore_parentheses = True
                use_ignore_asterisks = True
                use_ignore_angle_brackets = True
                use_remove_special_char = False
                config = None
            else:
                config = tts_preprocessor_config
                use_ignore_brackets = config.ignore_brackets
                use_ignore_parentheses = config.ignore_parentheses
                use_ignore_asterisks = config.ignore_asterisks
                use_ignore_angle_brackets = config.ignore_angle_brackets
                use_remove_special_char = config.remove_special_char
            ...
            async for sentence, display, actions in sentence_stream:
                if any(tag.name == "think" for tag in sentence.tags):
                    ...
                    tts = filter_text(
                        text=display.text,
                        remove_special_char=use_remove_special_char,
                        ignore_brackets=use_ignore_brackets,
                        ignore_parentheses=use_ignore_parentheses,
                        ignore_asterisks=use_ignore_asterisks,
                        ignore_angle_brackets=use_ignore_angle_brackets,
                    )

...
async def process_group_conversation(
    ...
    initiator_user_id: str = "default_user",
) -> None:
    ...
    Args:
        ...
        initiator_user_id: User ID of the conversation initiator
    """
    # 所有成员都使用同一个全局TTS管理器
    tts_managers = {uid: global_tts_manager for uid in group_members}
    ...
    try:
        ...
        for member_uid in group_members:
            ...
            # 使用发起者的user_id为发起者，其他成员使用默认值
            member_user_id = initiator_user_id if member_uid == initiator_client_uid else "default_user"
            store_message(
        ...
        # 确保发送结束信号，让所有成员状态正确转换
        try:
            for member_uid in group_members:
                member_ws_send = client_connections[member_uid].send_text
                broadcast_ctx = BroadcastContext(
                    broadcast_func=broadcast_func,
                    group_members=group_members,
                    current_client_uid=member_uid,
                )
                await finalize_conversation_turn(
                    tts_manager=tts_managers[member_uid],
                    websocket_send=member_ws_send,
                    client_uid=member_uid,
                    broadcast_ctx=broadcast_ctx,
                    context=client_contexts.get(member_uid)
                )
        except Exception as cleanup_error:
            logger.error(f"清理群组对话状态失败: {cleanup_error}")
...
async def handle_group_member_turn(
    ...
    full_response = await process_member_response(
        ...
        client_uid=current_member_uid,  # 传递客户端UID确保MCP工作区消息路由正确
    )
    ...
    try:
        ...
        if hasattr(context.agent_engine, 'chat') and hasattr(context.agent_engine.chat, '__code__'):
            ...
            if 'context_client_uid' in sig.parameters:
                ...
                logger.debug(f"🎯 群组对话调用Agent.chat时传递了client_uid: {client_uid}")
            else:
        ...
        async for output in agent_output:
            response_part = await process_agent_output(
                ...
                client_uid=current_member_uid,
                tts_priority=TTSPriority.NORMAL,  # 群组对话语音使用普通优先级
            )

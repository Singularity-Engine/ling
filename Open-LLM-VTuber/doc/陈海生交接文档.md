...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/multi_user/
```
...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/tools/
```
...
**使用示例：**
```python
from src.open_llm_vtuber.tools.tool_manager import tool_manager
...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/database/
├── pgsql/          # PostgreSQL 相关
└── redis/          # Redis 相关
```
...
**全局实例获取：**
```python
from src.open_llm_vtuber.database.pgsql.database_manager import (
    get_db_manager,
    get_session_manager,
    get_message_manager
)
...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/emotion_system/
```
...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/important/
```
...
### 📂 模块位置
```
Open-LLM-VTuber/src/open_llm_vtuber/utils/
```
...
```python
# PostgreSQL
from src.open_llm_vtuber.database.pgsql.init_database import init_database
init_database()

# Qdrant
from src.open_llm_vtuber.important.memories import init_db
init_db()
```

### 2. 启动 Agent
```python
from src.open_llm_vtuber.multi_user import UserSessionManager
from src.open_llm_vtuber.emotion_system import EmotionManager, EmotionalBasicMemoryAgent
from src.open_llm_vtuber.tools import tool_manager
...
### 4. 查看统计报告
```python
from src.open_llm_vtuber.utils.token_cost_tracker import token_cost_tracker
...
### 检查记忆存储
```python
from src.open_llm_vtuber.important.memories import list_all_memories_simple
...
### 验证 Token 统计
```python
from src.open_llm_vtuber.utils.token_counter import token_stats

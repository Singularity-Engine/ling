    ...
    switch (message.type) {
      ...
      case 'set-model-and-conf':
        ...
        if (message.model_info) {
          let modelUrl = message.model_info.url;
          if (!modelUrl.startsWith("http")) {
            // 相对路径，直接拼上 baseUrl
            modelUrl = baseUrl + modelUrl;
          } else {
            // 绝对路径（可能是 localhost），替换 host 部分为当前 baseUrl 的 host
            try {
              const urlObj = new URL(modelUrl);
              const baseUrlObj = new URL(baseUrl);
              urlObj.protocol = baseUrlObj.protocol;
              urlObj.host = baseUrlObj.host;
              modelUrl = urlObj.toString();
            } catch {
              modelUrl = baseUrl + message.model_info.url;
            }
          }
          // eslint-disable-next-line no-param-reassign
          message.model_info.url = modelUrl;

          // 覆盖模型定位参数 — 适配紧凑型布局（35vh Live2D 容器）
          // 原始 initialYshift=900 在全屏下合适，但在小容器里会把头推出画面
          // eslint-disable-next-line no-param-reassign
          message.model_info.initialYshift = -100;
          // eslint-disable-next-line no-param-reassign
          message.model_info.kScale = 0.65;
        }
